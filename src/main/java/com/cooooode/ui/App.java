...
public class App extends Application {
    ...
    public static String save_path = "./out";
    ///usr/java/project/LabelTool/src/main/resources/label.txt
    public static InputStream init_image = App.class.getClassLoader().getResourceAsStream("LabelTool.png");// "src/main/resources/LabelTool.png";
    public static InputStream init_label = App.class.getClassLoader().getResourceAsStream("label.txt");
    ...
    static {
        System.out.println(new Date() + " : [info] load predefined labels");
        try {
            File file = new File("label.txt");
            if (file.exists()) {
                System.out.println(new Date() + " : [info] find user's file label.txt");
                UiUtils.loadLabel(new FileInputStream(file));
            }
            UiUtils.loadLabel(init_label);
        } catch (IOException e) {
            e.printStackTrace();
        }
        System.out.println(new Date() + " : [info] create the default output folder");
        new File("out").mkdir();
    }
    ...
    public static void main(String[] args) {
        System.out.println(new Date() + " : [info] LabelTool started successfully");
        launch(args);
    }

    @Override
    public void start(Stage primaryStage) throws Exception {
        primaryStage.setTitle("LabelTool (Version 1.0)");
        ...
        group1.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
            public void changed(ObservableValue<? extends Toggle> ov,
                ...
                if (group1.getSelectedToggle() != null) {
                    ...
                    System.out.println(new Date() + " : [info] template style " + old_format + " -> " + format);
                }
            }
        });
        ...
        group2.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
            public void changed(ObservableValue<? extends Toggle> ov,
                ...
                if (group2.getSelectedToggle() != null) {
                    ...
                    System.out.println(new Date()+" : [info] preservation mode "+old_method+" -> "+method);
                }
            }
        });
